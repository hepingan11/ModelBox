<template>
	<view class="user-view-root">
		<scroll-view v-if="!isLoggedIn" scroll-y class="login-scroll">
			<LoginComponent @loginSuccess="handleLoginSuccess" />
			<!-- 底部占位确保 TabBar 不遮挡内容 -->
			<view style="height: 180rpx;"></view>
		</scroll-view>
		
		<scroll-view v-else scroll-y class="user-scroll" :class="themeClass">
				...
				<view class="section-card">
					<view class="menu-grid">
						...
						<view class="grid-item" @click="goToMessage">
							<view class="grid-icon-wrapper">
								<image src="/static/icon/message.png" class="grid-icon"></image>
							</view>
...
import LoginComponent from '@/pages/user/components/LoginComponent.vue'
...
.bg-blur-mask {
	...
	width: 100%;
	height: 150rpx;
	background: linear-gradient(to bottom, transparent, var(--bgColor2));
	z-index: 1;
}
