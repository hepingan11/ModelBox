...
import com.cn.bdth.config.PayConfig;
...
public class PayServiceImpl implements PayService {
    ...
    public List<ProductVo> getProductList() {
        return productMapper.selectList(
                ...
                .stream()
                .map(c -> new ProductVo().setProductId(c.getProductId()).setProductPrice(c.getProductPrice())
                        .setFrequency(c.getFrequency()).setProductName(c.getProductName()))
    ...
    private OrderPageVo getOrdersPage(final Long userId, final String prompt, final String state, final int pageNum) {
        ...
        final IPage<OrderPageVo.Orders> convert = ordersMapper
                .selectPage(new Page<>(pageNum, 15), new QueryWrapper<Orders>()
                        .lambda()
    ...
    public IPage<ProductVo> getProductPageVo(int pageNum, String prompt) {
        return productMapper.selectPage(new Page<>(pageNum, 15), new QueryWrapper<Product>()
                ...
                .convert(c -> new ProductVo().setProductId(c.getProductId()).setProductName(c.getProductName())
                        .setProductPrice(c.getProductPrice()).setFrequency(c.getFrequency())
    ...
    public AlipayPayCodeVo generatePayQrCode(final Long productId) {
        Product product = productMapper
                .selectOne(new QueryWrapper<Product>().lambda().eq(Product::getProductId, productId));
        if (product == null) {
        ...
        // 非必填 使用 响应字段中 url 就直接跳到百度了，如果访问，url_qrcode ，不会直接跳转，只有当支付完成后，再次刷新
        // url_qrcode中的连接，才会跳转。
    ...
    @Override
    public PayReturnInfoCommon submitPay(PayCommon payCommon) {
        ...
        amount.setTotal((int) (payCommon.getPrice() * 100));
        request.setAmount(amount);
        request.setAppid(appId);
        request.setMchid(mchId);
        request.setDescription(payCommon.getDescription());
        request.setNotifyUrl(payCommon.getNotifyUrl());
        String outTradeNo = LocalDateTime.now().toString().replace("-", "").replace(":", "").substring(2).replace(".",
                "");
        request.setOutTradeNo(outTradeNo);
        Payer payer = new Payer();
        payer.setOpenid(userMapper.selectById(UserUtils.getCurrentLoginId()).getOpenId());
        request.setPayer(payer);
        // 发送请求
        PrepayWithRequestPaymentResponse prepayWithRequestPaymentResponse = payConfig.service
                .prepayWithRequestPayment(request);
        return new PayReturnInfoCommon().setOutTradeNo(outTradeNo)
                .setPrepayWithRequestPaymentResponse(prepayWithRequestPaymentResponse);
    }
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public WxPayResponse createNativePay(Long productId) {
        // TODO设置支付结束时间并在前端确认
        ...
        outTradeNo = outTradeNo + UUID.randomUUID().toString().substring(0, 8);;
        amount.setTotal((int) (product.getProductPrice() * 100));
        request.setAmount(amount);
        request.setAppid(appId);
        request.setMchid(mchId);
        request.setOutTradeNo(outTradeNo);
        request.setDescription(product.getProductName());
        request.setNotifyUrl(webUrl + "/pay/native/callback");
        LocalDateTime createdTime = LocalDateTime.now();
        Orders orders = new Orders()
                .setOrdersId(outTradeNo)
                .setUserId(UserUtils.getCurrentLoginId())
        ...
        return new WxPayResponse()
                ...
                .setCodeUrl(payConfig.nativeService.prepay(request).getCodeUrl());
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public String nativePayCallback(final HttpServletRequest request) {
        try {
            // 解析回调通知
            Transaction transaction = payConfig.parseNotification(request);
            if (Transaction.TradeStateEnum.SUCCESS.equals(transaction.getTradeState())) {
                ...
                if (orders != null && orders.getState() == 0) {
                    ...
                    Long frequency = userMapper.getFrequencyById(orders.getUserId());
                    Product product = productMapper.selectOne(new QueryWrapper<Product>()
                            .lambda()
                            .eq(Product::getProductId, orders.getProductId())
                            .select(Product::getFrequency));
            ...
            log.error("Native回调处理失败", e);
            return "{\"code\": \"FAIL\", \"message\": \"失败\"}";
        }
    }

    @Override
    public String nativePaymentStatus(String orderId) {
        com.wechat.pay.java.service.payments.nativepay.model.QueryOrderByOutTradeNoRequest queryRequest = new com.wechat.pay.java.service.payments.nativepay.model.QueryOrderByOutTradeNoRequest();
        queryRequest.setMchid(mchId);
        queryRequest.setOutTradeNo(orderId);

        try {
            Transaction transaction = payConfig.nativeService.queryOrderByOutTradeNo(queryRequest);
            if (Transaction.TradeStateEnum.SUCCESS.equals(transaction.getTradeState())) {
                Orders orders = ordersMapper.selectOne(new QueryWrapper<Orders>()
                        .lambda()
                        .eq(Orders::getOrdersId, orderId));
                ...
                return "success";
            } else if (Transaction.TradeStateEnum.NOTPAY.equals(transaction.getTradeState())
                    || Transaction.TradeStateEnum.USERPAYING.equals(transaction.getTradeState())) {
                return "wait";
            }
        } catch (Exception e) {
            log.error("查询Native订单失败", e);
        }
        return "fail";
    }
